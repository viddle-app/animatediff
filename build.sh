...
build() {
    docker build . --tag "jfischjoffcanonicalllc/viddle-training"
}
...
push() {
    docker push "jfischjoffcanonicalllc/viddle-training:latest"
}

pull() {
    GHCR="jfischjoffcanonicalllc/viddle-training:latest"
    docker pull "$GHCR"
    docker tag "$GHCR" "$CWD"
}
...
run() {
    ...
    docker run --rm ${GPU_ARG} \
        ...
        -v "$PWD"/input:/home/huggingface/input \
        -v "$PWD"/output:/home/huggingface/output \
        "jfischjoffcanonicalllc/viddle-training:latest" "$@"
}
