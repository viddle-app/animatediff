...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if os.path.isfile(pretrained_model_path):
        ...
        teacher_unet = UNet2DConditionModel.from_pretrained(pretrained_model_path, subfolder="unet", torch_dtype=dtype)
        ...
        if image_finetune:
            ...
            if unet_checkpoint_path is not None and unet_checkpoint_path != "":
                ...
                teacher_unet = UNet3DConditionModel.from_unet2d(teacher_unet, 
                                                   unet_additional_kwargs=unet_additional_kwargs,)
                unet = UNet3DConditionModel.from_unet2d(unet, 
                                                   unet_additional_kwargs=unet_additional_kwargs,)
    ...
    if image_finetune == False and unet_additional_kwargs.motion_module_kwargs.zero_initialize == False:
        ...
        missing, unexpected = teacher_unet.load_state_dict(motion_module_state_dict, strict=False)
        assert len(unexpected) == 0
        print("missing", len(missing))
    ...
    alpha_schedule = alpha_schedule.to(local_rank)
    sigma_schedule = sigma_schedule.to(local_rank)
    solver = solver.to(local_rank)
    ...
    if not image_finetune:
        ...
        if os.path.isfile(pretrained_model_path):
            ...
            validation_pipeline.scheduler = EulerAncestralDiscreteScheduler(**OmegaConf.to_container(noise_scheduler_kwargs)),
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training):
                ...
                if not image_finetune:
                    ...
                    if alternate_losses: 
                        ...
                        loss = video_diff_loss_8(pred, latents.float(), anchor_frame_index=anchor_frame,)
                        ...
                        anchor_frame = torch.randint(0, video_length, (1,), device=latents.device).item()
                        loss = video_diff_loss_8(model_pred_float, 
                                                    target_float, 
