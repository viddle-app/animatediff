...
from src.utils.image_utils import save_gif_from_tensor, tensor_to_image_sequence

if is_wandb_available():
    import wandb
    wandb.init(project="animatediff-unet-fine-tune")
...
def compute_x_0(noise_scheduler, timesteps, noisy_latents, prediction):
...
def log_validation(noise_scheduler, 
                   step, 
                   output_folder, 
                   vae, 
                   text_encoder, 
    ...
    validate_timesteps = [999]


    # validate_pixel_values = rearrange(validate_pixel_values, "b c f h w -> (b f) c h w")
    # latents = vae.encode(validate_pixel_values).latent_dist
...
# 
    ...
    if len(combined_callback_images) > 0:
        ...
        image_folder_path = f"{output_folder}/{step}"
        tensor_to_image_sequence(combined.permute(1, 0, 2, 3), image_folder_path)
        log_data["latents"] = [
                        wandb.Image(combined, caption=f"{args.validation_prompts[0]}")
                    ]
...
def main():
    ...
    # get the config
    # copy it
    # add the additional channels
    # zero them out ... somehow
    additional_unet_kwargs = {"in_channels":8}
    ...
    if args.dataset_name is not None:
        ...
        # )
        # dataset_path = os.path.join(args.train_data_dir, args.dataset_config_name)  # Assuming 'args.train_data_dir' is the base path where the dataset was saved.
