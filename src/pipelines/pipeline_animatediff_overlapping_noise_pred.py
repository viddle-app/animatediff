...
from ..utils.partition_utils import indices_for_averaging, partition_wrap_around_2, partitions, partitions_wrap_around
    ...
    def prepare_latents(self, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, window_size, latents=None):
        ...
        if latents is None:
            ...
            if isinstance(generator, list):
                ...
                # repeat the latents for each frame, e.g. dim 2
                shape_with_one_frame = (batch_size, num_channels_latents, 1, height // self.vae_scale_factor, width // self.vae_scale_factor)
                latents = torch.randn(shape_with_one_frame, generator=generator, device=rand_device, dtype=dtype).repeat(1, 1, video_length, 1, 1).to(device)
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps * partition_count) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                indices = indices_for_averaging(video_length, window_length, shift_count)
                ...
                for partition_indices in indices:
                    ...
                    noise_pred_partition = self.unet(latent_model_input, 
                                        t, 
                    ...
                    # update the latents
                    values[:, :, partition_indices[0]:partition_indices[1]] += noise_pred_partition.to(cpu_device)
                    count[:, :, partition_indices[0]:partition_indices[1]] += 1

                # perform guidance
                if do_classifier_free_guidance:
                    noise_pred = torch.where(count > 0, values / count, values)
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)

                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_pred.to(device), t, latents, **extra_step_kwargs).prev_sample
