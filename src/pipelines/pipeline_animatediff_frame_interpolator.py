...
class StableDiffusionFrameInterpolatorPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        >>> from diffusers import StableDiffusionFrameInterpolatorPipeline
        ...
        >>> pipe = StableDiffusionFrameInterpolatorPipeline.from_pretrained(
    ...
    def prepare_image_latents(
        ...
        effective_batch_size = batch_size * num_images_per_prompt * video_length
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list):
                image_latents = [self.vae.encode(image[i:i+1]).latent_dist.mode() for i in range(effective_batch_size)]
                image_latents = torch.cat(image_latents, dim=0)
            else:
                image_latents = self.vae.encode(image).latent_dist.mode()

        if effective_batch_size > image_latents.shape[0] and effective_batch_size % image_latents.shape[0] == 0:
            deprecation_message = (
                f"You have passed {effective_batch_size} text prompts (`prompt`), but only {image_latents.shape[0]} initial"
